...
float horizontalAngle(int x) {
	...
	if (x < (FRAME_WIDTH / 2)) { //Left side
		return -((FOV_HEIGHT / 2) - angle);
	}
	else { //Right Side
		return angle - (FOV_HEIGHT / 2);
	}
}
	...
	if (hierarchy.size() > 0) {
		...
		if (numObjects<MAX_NUM_OBJECTS){
			...
			if (objectFound == true){
				...
				rectangle(cameraFeed, pt1, pt2, CV_RGB(255, 255, 255), 0.1);
				cout << rect.width << ": ";
				double distance = (rect.width - 139.8) / -9.6571;
				int width = calculateWidth(contours, indexFound);
				cout << width << ": ";

				cout << distance << ": H_Angle ";
				//draw object location on screen
				//drawObject(-1,-1,cameraFeed);
				float a = horizontalAngle(x);
				cout << a << ": ";


				float angle = verticalAngle(y);
				cout << "TEST " << angle << ": ";
				float d = calculateDistance(false, angle, rect.width);
				cout << d << endl;*/
...
string generateBroadcast() {
	...
	ss << "{" <<
		...
		"\"pillar2\":{\"distance\":" << objToString(pillar2d) << ",\"angle\":" << objToString(pillar2a) << "}," <<
...
int main(int argc, char* argv[])
{
	...
	while (1){
		...
		if (trackObjects) {
			...
			trackFilteredObject(yx,yy,ythresh,cameraFeed);
